...
if test "$PHP_AVDESK" != "no"; then
  ...
  DWAVDAPI_CONFIG=""
  AC_MSG_CHECKING(for dwavdapi 2.0.0 or greater)
  ...
  if test "x$DWAVDAPI_CONFIG" = "x"; then
     AC_MSG_ERROR(dwavdapi library version 2.0.0 or later is required to compile with Dr.Web AV-Desk support)
  fi
  
  DWAVDAPI_VERSION_NUM=`$DWAVDAPI_CONFIG --vernum`
  
  if test "$DWAVDAPI_VERSION_NUM" -ge 20000; then
